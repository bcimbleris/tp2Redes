...
int addrparse(const char *addrstr, const char *portstr,
    ...
    //padrao do protocolo da internet é 16 bits. faz cast de string para inteiro
    uint16_t port = (uint16_t)atoi(portstr); // unsigned short
    //foi passado um número errado, então finaliza o programa
    if (port == 0) {
        return -1;
    }
    //o numero do porto precisa estar em network byte order. Coloca o port em big endian, que é a representação correta
    port = htons(port); // host to network short

    //checa se é IPv4, se der erro tenta se é IPv6, se der erro de novo finaliza o programa
    ...
    struct in6_addr inaddr6; // 128-bit IPv6 address
        //tenta fazer o parser de IPv6 e se der certo coloca no inaddr6
    if (inet_pton(AF_INET6, addrstr, &inaddr6)) {
        struct sockaddr_in6 *addr6 = (struct sockaddr_in6 *)storage;
        addr6->sin6_family = AF_INET6;
        addr6->sin6_port = port;
        // addr6->sin6_addr = inaddr6
        //para poder pegar 16 bytes e copiar para outro lugar precisa ser feito dessa maneira em C
...
//recebe um sockaddr como parametro  o string onde vai ser printado e o tamanho do string
