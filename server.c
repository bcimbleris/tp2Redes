...
//recebe o tipo de protocolo do servidor e o porto onde ele vai ficar esperando e um exemplo
...
Coordinate coordServ = {-19.9927, -43.9451};
    ...
    //passa uma opção para o socket s reutilizar o porto caso o porto ja estiver sendo utilizado
    int enable = 1;
    ...
    //passa o socket e a quantidade de conexões que podem estar pendentes para tratamento
    if (0 != listen(s, 10)) {
        logexit("listen");
    }
    //imprime uma mensagem para falar que o bind deu certo e que o servidor está esperando conexãos
    char addrstr[BUFSZ];
    ...
    while (1) {
        ...
        //retorna um novo socket para falar com o cliente e armazena o endereço do cliente em caddr
        ...
        //envia a resposta para o cliente em cima do buf. limita para printar ate 1000 bytes e nao dar buffer overflow
        sprintf(buf, "remote endpoint: %.1000s\n", caddrstr);
        //envia no csock a mensagem
        count = send(csock, buf, strlen(buf) + 1, 0);
        //caso não tenha sido enviado o numero certo de dados fecha o programa
